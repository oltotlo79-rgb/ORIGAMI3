 ...
 * (動かさない側を示す点)を作る。側は「線の進行方向(始点→終点)に対する左右」
 * という畳み平面上の向きで表す。
 *
 * 注意: これは画面上の左右とは一致しない。カメラを回すと畳み平面の左右は画面の
 * 右にも左にも見えるため、画面に合わせるにはカメラの右方向ベクトルが要る。
 * この処理は3D描画に依存しない純粋な幾何計算として展開図区画・ストアからも
 * 呼ぶので、カメラを持ち込まない方針にした。代わりに画面側では「左/右」という
 * 言葉を使わず「こちら側/反対側」と呼び、どちらが動くかは3Dプレビューの
 * ハイライト(黄色)で見せる(ContextPanelの「動かす側」を参照)。
 */
