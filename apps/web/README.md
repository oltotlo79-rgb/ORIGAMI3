...
| 20,000ファイル | 公開直前に`apps/web/dist`の全ファイルを数え、20,000件超なら公開を止める。2026-08-31の実測は29件。 |
| 25 MiB/ファイル | 公開直前に26,214,400 bytes超の全ファイルを検出し、1件でもあれば公開を止める。現在の最大はWASMの7,479,744 bytes。 |

取扱説明書PDFは28,843,335 bytes（約27.5 MiB）で25 MiBを超えるため、Web成果物には入れません。「はい」を選んだときにGitHub Releasesの最新版`ORIGAMI3.pdf`を直接開きます。これにより、現在の成果物は29ファイル、合計11,575,746 bytes、25 MiB超0件です。WASMも公開直前の全ファイル検査に含めます。
