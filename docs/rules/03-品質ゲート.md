...
- 作業担当の「合格しました」という報告を鵜呑みにせず、Claudeが再実行結果を確認する。
...
- **#13 の一時 `#[ignore]` は解除済み**: `apps/desktop/src-tauri/src/surface_order_sa_endpoint_heavy.rs` で、edge 169 の面対22組だけが179.999°と入れ替わる（`changed_directions` 61→1）不具合を追いながら約16,000行のRustをコミットするため理由つき属性を付けていたが、2026-08-28に最適化ありで全110折り目・220方向を再測定し、`robust_stacks=4888`、`changed_directions=0` を確認して属性を外した。**#14 の一時 `#[ignore]` も解除済み**: 19本×2方向の再測定で `robust_stacks=1298`、`changed_directions=0` を確認して属性を外した。期待値・下限・許容差は変更していない。
- **#13・#14 について**: この2件は最適化なしでは手元で 476秒 / 175秒 かかる。CIの `checks` ジョブでは6件の`--skip`に含め、`performance`ジョブの最適化ありコマンドで実行する。手元で確かめるときも上表の最適化ありコマンドを使う。
