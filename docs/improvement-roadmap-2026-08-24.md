...
| MS | 内容 | 受け入れ基準 |
|---|---|---|
| M7 | FOLD 1.1/1.2共通限定profileのimport、FOLD 1.2のexport | FOLD 1.1を16件以上、FOLD 1.2を6件以上含む4出所30 fixtureを製品実行前に固定・分類し、panic 0、1.1であることだけを理由とする拒否0、対応範囲内の実数と対応範囲外の実数を報告する。対応範囲内は100%でtopology・B/M/V・折り角・step終点・層制約が許容差内で一致し、対応範囲外は100%で理由付き拒否とする。書出しの`file_spec`が1.2である率100%、file-open filter、書出し選択、取込結果/警告、Help、利用者向け説明書の5/5で「ほかの折り紙ソフトのファイル」と説明して内部用語表示0、対応外7項目の表示7/7、F/U入力の縮退警告率100%、警告付き取込の即時反映率100%、既存Undo 1回での復元率100%、失敗時の部分反映0、無言の破棄0とする。分類は製品結果を見た後に変更せず、件数合わせのために入力を壊さない。 |
