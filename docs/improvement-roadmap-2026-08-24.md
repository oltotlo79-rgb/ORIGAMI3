...
|---:|---|---|---|
| 1 | FOLD 1.1 / 1.2 | **共通限定profileを承認。取込は1.1と1.2、書出しは1.2。** 2D `vertices_coords`、edge topology、B/M/V、`edges_foldAngle`、表現可能な非循環 `faceOrders`、線形step frameを対象にする。F/Uは`Aux`へ縮退し、元指定を警告として残す。 | **F/U完全往復の41～62人日案は不採用。** 3D座標、枝分かれした手順、動画、名前付き技法の意味、注記、仕上げの丸み、FOLDのF/U区別は対象外。画面と利用者向け文書では「ほかの折り紙ソフトのファイル」と説明し、形式名や内部の構造名を表示しない。 |
...
施策8の全ての画面、警告、Help、書出し選択、利用者向け文書は、機能を**「ほかの折り紙ソフトのファイル」**として説明し、形式名や内部の構造名を表示しない。利用者から見える対応外一覧には、3D座標、枝分かれした手順、動画、名前付き技法の意味、注記、仕上げの丸み、元ファイルの「平ら」と「未指定」の区別、まだ平らになっていない途中の形で終わる手順のうち紙を曲げないと作れないものの8項目を省略なく出す。入力中のF/Uは内部で`Aux`へ縮退し、元assignmentとJSON pathを構造化警告へ保持する。画面では安全な日本語で伝え、黙って捨てる経路を1件も許さない。
...
この2変更と、施策8のFOLD-001～006およびM7受入基準は要件正本へ反映済みである。現行契約は`docs/requirements-definition.md:66,251,255-260,405`を正とする。
...
12. [施策8 FOLD 1.1 / 1.2 共通限定profileを実施する](#12-施策8-fold-11--12-共通限定profileを実施する)
...
1. FOLD 1.1 / 1.2共通限定profileは承認済みで、§0B.2/§12.2は要件正本へ反映済み。取込は1.1と1.2、書出しは1.2とする。
...
| 11 | 施策8 FOLD 1.1 / 1.2共通限定profile | 承認済み・要件正本反映済みで、30外部fixture、model/IPC/UIを横断する最大拡張。中核品質を閉じた後に行う。 | 競争上の交換形式 |

施策10のpolicy・監査CI、FOLDの3文書同期、施策9の準備は順序3～8と並行できる。これは担当を増やすだけで、同じファイルを同時編集してよいという意味ではない。施策12は見送りのため実装・検証順へ入れない。
...
## 12. 施策8 FOLD 1.1 / 1.2 共通限定profileを実施する

### 12.1 現要件と判断

要件正本上、FOLD 1.1/1.2共通限定profileの取込とFOLD 1.2の書出しは明示的な目標である。

- `docs/requirements-definition.md:66`: 取込では1.1と1.2を受け入れ、書出しは1.2とする共通限定profile。
- 同 `:251`: `### 6.9 FOLD 1.1 / 1.2 共通限定profile (FOLD)`。
- 同 `:255-260`: 有限かつ厳密な1.1/1.2、対応field、警告、即時反映、Undo 1回、失敗時不変を定める。
- 同 `:405`: 4出所30 fixture、版だけの拒否0、実数報告、画面5/5をM7受入基準とする。

2026-08-24、利用者はFOLD 1.2限定profileを承認し、2026-08-26、利用者決定「形式1.1は受け入れる」により取込を1.1/1.2共通へ拡張した。要件正本への反映は完了済みで、書出しは1.2のままとする。F/U完全往復の41～62人日案は不採用のまま変更しない。

### 12.2 正本反映済みの現行要件

#### §4.1へ追加する案

> - **FOLD 1.1および1.2共通限定profile**の取込と、**FOLD 1.2**の書出し（2D頂点、edge topology、B/M/V、`edges_foldAngle`、表現可能な非循環 `faceOrders`、線形step frame）。対応範囲、対応外の内容、縮退時の警告は§6.9 FOLD-001～006に従う。
...
| ID | 要件案 | 優先度 | 時期 |
|---|---|---|---|
| FOLD-001 | FOLD 1.1および1.2 JSONの2成分 `vertices_coords`、`edges_vertices`、`edges_assignment` を読み込み、FOLD 1.2 JSONとして書き出し、正方形または長方形の単一紙へ変換する。取込時の`file_spec`は有限かつ厳密に1.1または1.2であるものだけを受け入れ、それ以外の版は対応外とする。edge topologyとB/M/Vを保持する。3成分の頂点座標は対応外とする。 | MUST | M7 |
...
| FOLD-005 | 画面と利用者向け文書では、この共通profileを「ほかの折り紙ソフトのファイル」と説明し、`FOLD 1.1`、`FOLD 1.2`、`schema`、`parser`、`validator`、`faceOrders`、`frame`、`Aux`、JSON pathなどの内部用語を表示しない。3D座標、枝分かれした手順、動画、名前付き技法の意味、注記、仕上げの丸み、元ファイルの「平ら」と「未指定」の区別を保持できないこと、まだ平らになっていない途中の形で終わる手順のうち紙を曲げないと作れないものの8項目を、利用者が理解できる日本語で表示する。入力中のF/Uは内部では`Aux`へ縮退し、元assignmentとJSON pathを`FoldIssue`へ全件保持するが、画面では安全な日本語へ変換する。無言で捨てる件数を0とする。 | MUST | M7 |
...
- `ori3-export` の責務を「SVG/PNG/折り図PDF生成、およびFOLD 1.1/1.2共通限定profileの中立なimportとFOLD 1.2のexport変換」へ改訂する。既存crateへimport責務を加える以上、この行を変えずに実装しない。
- FOLD formatが増えて依存や責務の分離根拠が得られた場合だけ、`ori3-interchange` の新設を別の要件・Cargo承認として判断する。最小profileでは新crateを前提にしない。

#### §9.3 IPC表を変える案

- `document_open` を `.ori3` とFOLD 1.1/1.2共通限定profileの読込（形式enumまたは確実な拡張子/内容判定）へ拡張する。FOLD時は `DocumentStore::import_fold` を呼び、dirtyな未保存作品として返す。
...
| MS | 内容 | 受け入れ基準 |
|---|---|---|
| M7 | FOLD 1.1/1.2共通限定profileのimport、FOLD 1.2のexport | FOLD 1.1を16件以上、FOLD 1.2を6件以上含む4出所30 fixtureを製品実行前に固定・分類し、panic 0、1.1であることだけを理由とする拒否0、対応範囲内の実数と対応範囲外の実数を報告する。対応範囲内は100%でtopology・B/M/V・折り角・step終点・層制約が許容差内で一致し、対応範囲外は100%で理由付き拒否とする。書出しの`file_spec`が1.2である率100%、file-open filter、書出し選択、取込結果/警告、Help、利用者向け説明書の5/5で「ほかの折り紙ソフトのファイル」と説明して内部用語表示0、対応外8項目の表示8/8、F/U入力の縮退警告率100%、警告付き取込の即時反映率100%、既存Undo 1回での復元率100%、失敗時の部分反映0、無言の破棄0とする。分類は製品結果を見た後に変更せず、件数合わせのために入力を壊さない。 |
...
| 区分 | できること | できないこと |
|---|---|---|
| 実装途中のCP交換pilot | 2D頂点、edge topology、B/M/Vのexact往復、F/Uの警告付き`Aux`縮退、正方形・長方形、外部FOLD 1.1/1.2 toolとの基本CP交換 | `edges_foldAngle`、`faceOrders`、step frameをまだ検査しないため、施策8の完成や製品公開にはならない。 |
| **承認済みのレビュー適合最小profile** | 上記に`edges_foldAngle`、表現可能な非循環`faceOrders`、線形step frameを加え、step終点まで往復する。 | 3D座標、枝分かれした手順、動画、名前付き技法の意味、注記、仕上げの丸み、FOLDのF/U区別、まだ平らになっていない途中の形で終わる手順のうち紙を曲げないと作れないもの。これら8項目は必ず利用者へ表示する。 |
...
| 実パス | 実関数・計画関数 | 作業 |
|---|---|---|
| `docs/requirements-definition.md` | 関数名は**該当なし**。要件正本の条文・表・milestoneであり実行コードではない。 | 現行正本との同期を確認し、`docs/implementation-roadmap.md`、本書、`docs/progress.md`の差分を閉じる。 |
...
|---|---|---|---|---|
| 8-A 承認反映 | **1件、非コード**（1～2人日） | **terra:** 利用者が決めたprofileを要件・roadmap・progress差分へそのまま反映する文書作業で、新しい範囲判断はしない。 | 正本反映済み。`docs/implementation-roadmap.md`、本書、`docs/progress.md`を現行条文へ同期する。 | 採用profile、条文差分、FOLD-001～006/M7の100%対応、未決事項0。 |
...
9. file-open filter、書出し選択、取込結果/警告、Help、利用者向け説明書の5/5で、利用者へ**「ほかの折り紙ソフトのファイル」**と説明する。`FOLD 1.1`、`FOLD 1.2`、`FOLD対応`、`FOLD完全対応`などの形式名・内部用語の表示は0件。
...
- 要件正本または3文書の同期を未反映のまま行うコード、Cargo構成、file filter、ExportKind変更。
...
- **依存:** 利用者の範囲承認は2026-08-24、形式1.1の取込承認は2026-08-26に完了し、要件正本へ反映済みである。3文書を同期し、施策1、3、4、6完了後に実装する。
- **リスク:** `faceOrders` と現行modelが非同型。表現不能なfileは対応外としてpath付きで拒否し、意味を近似しない。schema拡張や承認profile縮小が必要になった場合は停止し、新しい利用者判断へ戻す。
- **リスク:** 利用者向け説明が全仕様を扱うように誤読されること。5表示面の説明100%、対応外7/7、F/U警告100%のいずれか1件でも欠けたら公開しない。
- **点数:** 要件正本は改訂済みだが、全実装・検査が未完の現在は+0。全実装・検査後は**推測**でEXP +0.5、SYS +0.5、Docs +0.5。レビュー上は12分野点より競争劣位解消の意味が大きい。
...
- 折り目の組み合わせによって途中でつまみを動かしても同じ形に見えることがある。この限界を内部用語のない日本語で常時示し、割合どおりに連続して変わったと誤認させない。
...
5. 一斉折り中の中立表示、表裏、輪郭、警告、重なり順を確定と見せない表示を、折り鶴、やっこさん、カエル、鳥の基本形の4標本×0/25/50/75/100%の画像20/20で確認する。各画像は1000×700、同じ開始姿勢・同じ視点とし、実行ファイル、標本、割合、実行ファイルSHA-256を記録する。20/20を原寸で見て、面欠落0、内部用語0、診断に対応する日本語警告の表示100%、操作阻止0を確認する。単独変更の作業ツリーから撮影し、未実行を合格と書かない。
6. 上記4標本を製品経路で0→25→50→75→100%の順に操作し、16隣接区間すべての固定3D領域の変更画素数と最大実角差を記録する。変更画素500px以上かつ最大実角差1e-9°超を「形が変わった区間」とし、11/16以上とする。各標本で0→50の前半と50→100の後半にそれぞれ1区間以上の変化を必須とし、3割合以上が連続して同じ形になる件数は0とする。形が変わらない区間は許すが、その可能性を内部用語のない日本語で常時表示する。

### 15.4.1 再開2の実行集計（2026-08-26）

次表は、性能を測る検査と実機画像を含めずに再確認した実行結果である。未実行を合格として数えない。

| 単位 | 再開2の実行結果 | 合格条件に対する状態 |
|---|---|---|
| 11-A | `commands::tests::fold_all_three_fixtures_are_finite_flat_and_bit_deterministic` と `fold_all_frog` は合格。折り鶴・鳥の基本形・やっこさん・カエルの4標本×5割合×10回で、有限値・面完全・符号を確認した。既存3標本は0%平面・bit一致も確認する。 | **200/200**。カエルは248本の山谷ヒンジを対象にした50出力である。 |
| 11-B | `foldAllPreview.test.ts` は23/23合格。1秒120入力・IPC 65 call以下・同時実行最大1・最後の入力120/120採用と、古い応答を復帰後に採用しない経路を実行した。 | 数値契約の自動検査は合格。製品経路の画像・性能は別に集計する。 |
| 11-C | `FoldAllPreview.dom.test.tsx` の性能1件を除く12/12と、`keyboardOnly.usability.test.tsx` 4/4が合格。0/100%・1%刻み、入口、戻る操作、常時表示を確認した。 | 1000×700、キーボード導線、タイムライン一時表示の実機受入出力は **0/3**。 |
| 11-E | `foldAllPreview.test.ts` 23/23で保存、作品Undo/Redo非進行、復帰中保存、遅延応答の破棄を確認した。 | 保存前後Document 20/20、性能、単独変更ツリーの画像20/20を含む1集計は未完了。 |

33msの性能検査と4標本×5割合の画像20/20は、統括が計算機の空きを宣言した後にだけ実行する。

<!-- 2026-08-26の固定matrix実測は変化14/16区間、変化区間の最小画像差634px。§10.7.9に従い、境界は実測を直接使わず、14×0.8=11.2から11区間（11/14=78.57%）、634×0.8=507.2から500px（500/634=78.86%）とした。最大実角差1e-9°超とのANDで、再描画だけの画素差を変化に数えない。 -->
...
| 6 bundle | 6 | 6 | 8～13 |
| 7 文書機械検証 | 4 | 13 | 12～23 |
| 8 FOLD 1.1 / 1.2共通限定profile | 6 | 15 | 33～48 |
| 9 実利用者 | 3 | 8 | 13～21 |
...
### 17.3 進捗の見える化（2026-08-26再集計）

この表は今版のリリース範囲を母数とする。**以後、担当の完了報告ごとに、その施策の「完了した単位」と「状態」を同じ更新で直し、合計と完了率を再計算する。**

**最終数え直し:** 2026-08-26 04:07 JST。`scratchpad/roadmap-recount-report.md` の単位別根拠（実測値又は実施済み検査）に基づく。未コミットの証拠は同報告で明記している。

**更新漏れの事実:** 上の更新規則があるにもかかわらず、2026-08-24以降に担当の完了報告が複数あった後も、§17.3は1回も更新されず、5/106（4.7%）のままだった。

**再発防止の仕組み案（未実装）:** `scripts/check-report-log.ps1` と `scripts/generate-current-status.ps1` と同型の `scripts/check-roadmap-recount.ps1` を通常checksへ追加する。機械可読の単位台帳（unit ID、状態、見積、合格値、実測/検査exit、証拠path・commit・未コミットflag）を、追跡済み入力だけの隔離snapshotで2回読む。(1) 完了には合格値を満たす実測又はexit 0を必須にし、`-Check`では未コミット証拠を拒否する、(2) ID重複0、施策別の状態数、合計106、完了率を検査する、(3) 正規化した§17.3用Markdown markerとのhash・最初の差分行を照合する、(4) `docs/報告記録.md` の完了報告と台帳更新を対応付ける、(5) unit ID・source path・selectorを出す故意差分fixtureを置く。違反時はexit non-zeroにする。

| 施策 | 見積(人日) | 単位数 | 完了 | 実行中 | 止まっている | 未着手 |
|---|---:|---:|---:|---:|---:|---:|
| 1 提案負荷非依存 | 11～16 | 5 | 4 | 0 | 1 | 0 |
| 2 原子的MoveStep | 5～8 | 3 | 2 | 1 | 0 | 0 |
...
| 8 FOLD 1.1 / 1.2共通限定profile | 33～48 | 15 | 7 | 0 | 1 | 7 |
| 9 実利用者（今版の準備） | 6～9 | 3 | 1 | 1 | 1 | 0 |
| 10 supply-chain | 11～17 | 6 | 0 | 0 | 6 | 0 |
| 11 一斉折り案B | 8～12 | 5 | 3 | 1 | 1 | 0 |
| 12 Linux対応 | 0 | 0 | 0 | 0 | 0 | 0 |
| **今版リリース合計** | **177～269** | **106** | **74** | **8** | **14** | **10** |
| 次の版へ繰越: 施策9の外部実施 | 7～12 | 5 | — | — | — | — |

単位ベースの完了率は**74 / 106 = 69.8%**である。人日ベースの完了率は、複数単位に対する一括見積を単位へ推測配分できないため、厳密には未算出とする。残り32単位の施策別見積を合算した作業見通しは**63～92人日**で、うち利用者協力待ちは8単位・**16～24人日**である（待機期間は含めない）。
...
| 8 | 要件正本との3文書同期、1、3。実装推奨は4/6後 | corpus取得/license確認 |
...
- 改善ロードマップ全体（今版と次の版を合わせた完了）は、各施策の数値条件が全て合格し、未実行0となったときである。施策9は次の版で外部参加者15名を完了するまで、ロードマップ全体の完了にしない。ただし今版のリリース条件は§17.2に定めた準備完了までとする。施策11は案Bの一時表示をN/Aにせず、手順非記録・非保存・非Undoと画面表示を含めて合格させる。FOLD 1.1/1.2共通限定profileの取込とFOLD 1.2書出しは採用済みでN/Aにせず、自動更新は施策10の初めから範囲外なのでN/A項目を作らない。施策12は見送り中のため完了対象に数えない。
...
本書は0.5.0の現行ソースを起点にした実行計画である。0.4.5レビューの解決済み3件を再作業へ戻さず、残る大域progressと壁時計結果経路、原子的MoveStep、一般化証拠、保守境界、利用者・配布証拠を順に閉じる。2026-08-24の利用者決定によりFOLD 1.2限定profileと供給網の監視・公開を承認し、2026-08-26の利用者決定により取込をFOLD 1.1/1.2共通へ拡張した。実利用者検証は準備のみ承認済みであり、自動更新とF/U完全往復は範囲外である。
