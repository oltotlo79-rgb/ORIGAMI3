...
/// 「**指定どおりには折れなかった**」ことを表す警告の言い回し。
///
/// 技法が出す警告のほとんどは、**指定どおりに折ったうえでの注意**である
/// (「止めずに警告する」= `docs/requirements-definition.md` §2 / `CLAUDE.md` §8)。
/// たとえば「反対向きの折り線が既にあります(**折り上がりは同じですが**…)」は、
/// 実際の紙では普通に折れるので、警告を理由に折るのをやめてはいけない。
///
/// ここに並べるのは、**折り上がりが指定と違ってしまう**ものだけである。
/// 判定を文言と同じ場所に置いているのは、文言を書き換えたときに
/// ここを直し忘れないようにするため([`TEAR_MARK`] と同じやり方)。
/// 対になる検査は `crates/ori3-layers/tests/technique_warnings.rs`。
pub const NOT_AS_REQUESTED_MARKS: [&str; 5] = [
    // 山谷と紙の重なり順が食い違い、記録した手順から折り直すと形が変わる
    "展開図から折り直したときに形が変わります",
    // 中心線から支点を決められず、動きそのものを作れなかった
    "支点が決められません",
    // 新しくできた面をどこへ動かすか決められず、置き去りにした
    "親面が特定できないため",
    // 指定した層が実行から外された(頼んだ手と別の手になる)
    "現在の面に存在しないため除外しました",
    "折り線の可動側に掛かっていないため除外しました",
];

/// この警告は「指定どおりには折れなかった」ことを表すか。
///
/// `false` のときは、**折る動き自体は指定どおりに作れている**。
/// 実際に折れるかどうかは、警告の文面ではなく
/// 折り途中の姿勢(裂け・すり抜け・面の欠け)で判断する。
#[must_use]
pub fn warning_means_the_fold_was_not_as_requested(warning: &str) -> bool {
    NOT_AS_REQUESTED_MARKS
        .iter()
        .any(|mark| warning.contains(mark))
}
