...
/// 現在位置から層の接触を探す。戻り値は(下の層の頂点, 上の層の頂点, 押し戻す向き,
/// その間が袋の内側か)。一様格子で近傍を拾い、層番号が[`LAYER_DIFF`]以上離れた
/// 組だけを拾う(折り線上の共有点は隣接層の中間値になるので外れる)。
fn find_contacts(
    pos: &[DVec3],
    up: &[DVec3],
    c: &Constraints,
) -> Vec<(u32, u32, DVec3, bool)> {
    let (lmin, lmax) = c
        .layer
        .iter()
    ...
    for (i, p) in pos.iter().enumerate() {
        ...
        for d in NEIGHBOR_CELLS {
            ...
            for &j in grid.cell([b[0] + d[0], b[1] + d[1], b[2] + d[2]]) {
                ...
                out.push((lo, hi, dir, is_sealed(&c.sealed, li.min(lj), li.max(lj))));
            }
        }
    }
    out
...
pub(crate) fn run(
    ...
    for it in 0..iterations {
        if it % REBUILD_EVERY == 0 {
            ...
            contacts = find_contacts(pos, &up, c);
            if stack.is_empty() {
                stack = contacts.iter().map(|&(a, b, _, s)| (a, b, s)).collect();
            }
        ...
        for &(a, b, d, _) in &contacts {
            separate(pos, a, b, d);
        }
    }
    contacts
        .iter()
        .filter(|&&(a, b, d, _)| (pos[b as usize] - pos[a as usize]).dot(d) < 0.0)
        .count()
}
