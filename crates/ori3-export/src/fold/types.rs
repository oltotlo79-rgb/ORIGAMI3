...
/// 利用者から1操作以内で到達できる場所へ示す、対応外の8項目。
///
/// 8項目目は2026-09-05に足した。それまで、終点が平坦かどうかに関わらず全ての手順へ
/// 「紙を曲げずに指定の角へ到達できること」を要求しており、伝承の折り鶴のように
/// 紙を曲げずには折れない作品（`docs/rules/03-品質ゲート.md` §7.1）を書き出せなかった。
/// 終点が平坦な手順は宣言角の平坦再生で確かめるよう直したので、この制限は
/// 「終点が平坦でない手順」だけに残る。残った範囲を隠さず、ここへ明記する。
pub const FOLD_1_2_UNSUPPORTED_FEATURES: [&str; 8] = [
    "3D座標",
    "枝分かれした手順",
    "動画",
    "名前付き技法の意味",
