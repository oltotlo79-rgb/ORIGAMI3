...
pub fn validate(cp: &CreasePattern) -> Vec<String> {
    ...
    for e in cp.edges.iter().filter(|e| e.kind != EdgeKind::Aux) {
    ...
    let vpos: BTreeMap<VertexId, DVec2> = cp
        ...
        .collect();
    let mut segs: Vec<(EdgeId, DVec2, DVec2)> = cp
        .edges
        .iter()
        .filter(|e| vpos.contains_key(&e.v0) && vpos.contains_key(&e.v1))
        .map(|e| (e.id, vpos[&e.v0], vpos[&e.v1]))
        .collect();
